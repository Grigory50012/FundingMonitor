...
// Local default coins (shared across UI)
...
const DashboardContainer: React.FC = () => {
  // Filters (persisted via localStorage)
  ...
  // Load available coins (defensive: keep existing behavior)
  ...
  const availableCoins = Array.from(new Set([...DEFAULT_COINS, ...allCoins])).sort();

  // Render presentational UI
  return (
    <div className="min-h-screen text-[var(--tg-text)]" style={{ backgroundColor: "var(--tg-bg)" }}>
      <main className="max-w-[1920px] mx-auto p-6">
              ...
              {isLoadingCurrent ? (
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    <div className="w-12 h-12 border-4 rounded-full animate-spin mx-auto mb-4" style={{ borderColor: "var(--tg-border)", borderTopColor: "var(--tg-button)" }} />
                    <p style={{ color: "var(--tg-text-secondary)" }}>Загрузка текущих данных...</p>
                  </div>
                </div>
              ) : (
                <CurrentDataTable data={currentData} selectedExchanges={mainFilters.exchanges} />
              )}
            </div>
          </div>
          ...
          >
            ...
            <div className="flex-1 min-h-0">
              {isLoadingHistory ? (
                ...
                <HistoryPanel data={historyData} selectedExchanges={mainFilters.exchanges} timeRange={timeRange} onTimeRangeChange={setTimeRange} />
              ) : (
                <HistoryTable symbol={mainFilters.symbol} selectedExchanges={mainFilters.exchanges} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-2xl p-6 overflow-hidden flex flex-col h-[500px]" style={{ backgroundColor: "var(--tg-bg-secondary)", border: "1px solid var(--tg-border)" }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              <h2 className="text-lg font-semibold flex items-center gap-2">Арбитражные возможности</h2>
              <CompactFilter
                selectedExchanges={arbitrageFilters.exchanges}
                onExchangesChange={(exchanges) => setArbitrageFilters({ ...arbitrageFilters, exchanges })}
                selectedSymbol={arbitrageFilters.symbol}
                onSymbolChange={(symbol) => setArbitrageFilters({ ...arbitrageFilters, symbol })}
                availableSymbols={availableCoins}
              />
            </div>
            <button onClick={() => refreshArbitrage?.()} disabled={isLoadingArbitrage} className="px-3 py-1.5 text-sm rounded-xl font-medium transition-all flex items-center gap-1.5 disabled:cursor-not-allowed" style={{ backgroundColor: isLoadingArbitrage ? "var(--tg-hint)" : "var(--tg-button)", color: "var(--tg-button-text)", opacity: isLoadingArbitrage ? 0.6 : 1 }}>
              <svg className={`w-3.5 h-3.5 ${isLoadingArbitrage ? "animate-spin" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
            </button>
          </div>
          <div className="flex-1 min-h-0">
            {isLoadingArbitrage ? (
