...
- Русские строки в коде и документации проверены на mojibake: файлы проходят strict UTF-8, source/dist не содержат типичных mojibake-маркеров. Если проблема проявляется только в PowerShell-выводе, это считается особенностью консольного отображения, а не дефектом runtime UI.
